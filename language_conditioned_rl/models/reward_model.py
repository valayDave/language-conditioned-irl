...
class LGRInferenceMixin(object):
    ...
    def encode_trajectory(self, act_list, state_list):
        ...
        return (
                ...
                self.make_mask_from_len(torch.Tensor([state_list_len]), self.max_traj_length),
                action_tensor.unsqueeze(0),
                self.make_mask_from_len(torch.tensor([act_list_len]), self.max_traj_length)
            )
    ...
    def from_neptune(cls,\
        ...
        checkpoint = torch.load(os.path.join(base_path,ckck_name),map_location=torch.device('cpu'))
        ...
        # print(config)
        trans = cls(**config,experiment_name=experiment_name,loaded_checkpoint=checkpoint_path)
