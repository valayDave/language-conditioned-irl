...
class CrossModalAttentionWithMask(nn.Module):
    ...
    def forward(self, seq_x, seq_y, mask=None, return_attentions=False):
        '''
        r(seq_x|seq_y)
        r(action|text)
        r(text|action)
        ...
        q = einops.rearrange(self.q_layer(
            seq_x), 'b s (h d) -> b h s d', h=self.num_heads)
        k, v = map(lambda x: einops.rearrange(
            x, 'b s (h d) -> b h s d', h=self.num_heads), kv)
        scaled_dot_product = torch.einsum('bhsd,bhnd->bhsn', k, q) * self.scale
        # add mask here for better performance. gi
        if mask is not None:
            # print(f"Shape Of Mask {mask.shape}")
            mask_vals = self.get_extended_attention_mask(
                mask, seq_x.size(), device=seq_x.device)
        ...
        reweighted_sequence_embedding = einops.rearrange(
            value_weighted_sum, 'b h s d -> b s (h d)', h=self.num_heads)
        if not return_attentions:
            return self.ff_layer(reweighted_sequence_embedding)
        else:
            # Return output,attention_matrix
            return self.ff_layer(reweighted_sequence_embedding), weighted_sum
...
class CrossModalAttentionWithMultChannelMask(CrossModalAttentionWithMask):
    def __init__(self, hidden_size, dropout=0.1, num_heads=4, scale=0.2):
        super().__init__(hidden_size, dropout=dropout, num_heads=num_heads, scale=scale)

    def apply_mask(self, seq_x: torch.Tensor, seq_y: torch.Tensor, scaled_dot_product: torch.Tensor, mask_x=None, mask_y=None):
        attnx = None
        attny = None
        if mask_x is not None:
            attnx = self.get_extended_attention_mask(
                mask_x, seq_x.size(), device=seq_x.device)
        if mask_y is not None:
            attny = self.get_extended_attention_mask(
                mask_y, seq_y.size(), device=seq_x.device)
    ...
    def forward(self, seq_x, seq_y, mask=None, mask_y=None, return_attentions=False):
        ...
        scaled_dot_product = self.apply_mask(
            seq_x, seq_y, scaled_dot_product, mask_x=mask_x, mask_y=mask_y
        )
        weighted_sum = F.softmax(scaled_dot_product, dim=-1)
        value_weighted_sum = torch.einsum('bhsn,bhsd->bhnd', weighted_sum, v)
        reweighted_sequence_embedding = einops.rearrange(
...
class MultiModalAttentionBlock(nn.Module):
    def __init__(self,
        ...
        self.attn = CrossModalAttentionWithMultChannelMask(
            hidden_size, num_heads=num_attention_heads, scale=scale, dropout=attn_pdrop)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=layer_norm_epsilon)
        self.mlp = Residual(
            MLP(inner_dim, embedding_size=embedding_size, resid_pdrop=resid_pdrop))

    def forward(
        self,
        seq_x, seq_y, mask=None, mask_y=None, return_attentions=False
    ):
        if not return_attentions:
            attn_outputs = self.attn(
                self.ln_1(seq_x), self.ln_1(seq_y), mask=mask, mask_y=mask_y
            )
            # Residual connection
            ...
            attn_outputs, attn_vals = self.attn(
                self.ln_1(seq_x), self.ln_1(seq_y), mask=mask, mask_y=mask_y, return_attentions=return_attentions
            )
            # Residual connection
            attn_outputs = seq_x + attn_outputs
            feed_forward_hidden_states = self.mlp(self.ln_2(attn_outputs))
            # return ff_values + attenion_state_at_layer
            return feed_forward_hidden_states, attn_vals
