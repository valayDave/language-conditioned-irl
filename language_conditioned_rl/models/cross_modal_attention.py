...
class MultiModalAttentionBlock(nn.Module):
    def __init__(self,
        ...
        self.attn = CrossModalAttentionWithMask(
    ...
    def forward(
        self,
        seq_x, seq_y, mask=None, return_attentions=False
    ):
        if not return_attentions:
          ...
          attn_outputs, attn_vals = self.attn(
              self.ln_1(seq_x), self.ln_1(seq_y), mask=mask, return_attentions=return_attentions
          )
          # Residual connection
          attn_outputs = seq_x + attn_outputs
          feed_forward_hidden_states = self.mlp(self.ln_2(attn_outputs))
