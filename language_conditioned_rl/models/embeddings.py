...
class StateEmbedding(nn.Module):
  def __init__(self,
    ...
    super().__init__()
    self.linear_transform = nn.Linear(state_dims,hidden_dims)
  
  def forward(self,state): # state should be state_dims
    return self.linear_transform(state) # Simple Linear transformation of the state variables 


class ActionEmbedding(nn.Module):
  def __init__(self,
               num_actions = 3,
               action_type='discrete', # 'discrete' | 'continous'
               hidden_dims = 256,
               ):
    super().__init__()
    if action_type == 'discrete':
      print(f"Action Embedding Size {num_actions}")
      self.embd = nn.Embedding(num_actions,hidden_dims)
    else:
      self.embd = nn.Linear(num_actions,hidden_dims)

  def forward(self,action): 
    return self.embd(action)
